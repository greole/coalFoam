...
#include "relativeVelocityBase.H"
#include "polyMesh.H"
#include "SubField.H"
...
Foam::relativeVelocityBase::relativeVelocityBase
(
    const polyMesh& mesh,
    ...
    sumTriMagSf_(Pstream::nProcs() + 1, 0.0)
{
    if (patchId_ < 0)
    {
        FatalErrorIn
        (
            "relativeVelocityBase::relativeVelocityBase"
            "("
                "const polyMesh& mesh, "
                "const word& patchName"
            ")"
...
Foam::relativeVelocityBase::relativeVelocityBase(const relativeVelocityBase& pib)
:
...
void Foam::relativeVelocityBase::updateMesh(const polyMesh& mesh)
{
...
void Foam::relativeVelocityBase::setPositionAndCell
(
    const polyMesh& mesh,
    cachedRandom& rnd,
    vector& position,
...
)
{
    ...
    if (cellOwners_.size() > 0)
    {
        ...
        if (Pstream::myProcNo() == procI)
        {
            ...
            const scalar a = SMALL;//;rnd.position(scalar(0.1), scalar(0.5));
